    ...
    <div>
        <div class="order-box">
            <div class="menu-box" ref="menuBox">
                <ul>
                    <li class="menu-list" v-for="(item,index) in productList" :key="index"
            ...
            <div class="prod-box" ref="proBox">
                <ul>
                    <li class="cate-list" v-for="(item,typeIndex) in productList" :key="typeIndex" >
                        ...
                        <ul>
                            <li class="prod-list" v-for="(prop,index) in item.content" :key="prop.id">
                                ...
                                <div class="prod-info">
                                    ...
                                    <div class="sale">
                                        <span class="sale-span">月销:{{prop.num}}</span>
                                        <span>赞:{{prop.up}}</span>
                                        </div>
                                    <div class="price">价格:{{prop.price}}</div>
...
import addCart from './../../components/add-cart'
    export default {
        ...
        methods:{
            initScroll(){
                this.menuScroll =new BScroll('.menu-box' ,{bounce:false,click:true});
                this.prodScroll =new BScroll('.prod-box' ,{bounce:false,click:true,probeType:3});
                //获取右侧每个分类的垂直方向位置
                this.getPosY();
                //获取左侧li列表
